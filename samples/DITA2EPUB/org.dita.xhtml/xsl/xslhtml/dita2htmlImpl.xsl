...
<xsl:template match="*" mode="dita2html:get-default-fig-class">
  ...
  <xsl:template match="*[contains(@class,' topic/fig ')]" mode="fig-fmt">
    ...
    <xsl:apply-templates select="*[contains(@class,' ditaot-d/ditaval-startprop ')]" mode="out-of-line"/>
    <!-- OXYGEN PATCH START  EXM-18109 Put the image content in a DIV so it can be styled from CSS. -->
    <div>
...
<!-- OXYGEN PATCH START  EXM-33743 Skip img/data-about elements.  -->
<xsl:template match="*[contains(@class,' topic/data-about ')]"/>
<!-- OXYGEN PATCH END  EXM-33743  -->
...
<xsl:template name="topic-image">
  <!-- OXYGEN PATCH START  Use <img> for SVG images too. -->
  <xsl:apply-templates select="*[contains(@class,' ditaot-d/ditaval-startprop ')]" mode="out-of-line"/>
  <img>
    <xsl:call-template name="commonattributes">
  ...
  <!-- OXYGEN PATCH END  Use <img> for SVG images too. -->
  <xsl:apply-templates select="*[contains(@class,' ditaot-d/ditaval-endprop ')]" mode="out-of-line"/>
</xsl:template>
...
<xsl:template name="output-stentry-id">
  ...
  <xsl:choose>
    <xsl:when test="@id">    <!-- If ID is specified, always use it -->
      <!-- EXM-28774 OXYGEN PATCH FOR CELL. -->
      <xsl:attribute name="id">
    ...
    <xsl:variable name="childlang">
      <xsl:choose>
        <!-- Update with DITA 1.2: /dita can have xml:lang -->
          <!-- OXYGEN PATCH  EXM-30897 -->
          <xsl:when test="self::dita[not(@xml:lang)] | *[contains(@class, ' map/map ')][@xml:lang]">
